...
assertTrue("App test-app not found", apps.size() > 0)
app = apps.get(0)

deployments = opsworks.describeDeployments(new DescribeDeploymentsRequest().withStackId(stack.getStackId()).withAppId(app.getAppId())).getDeployments()

assertEquals("# of deployments", 1, deployments.size())
